...
import { GROUPED_CIPHERTEXT_2_HANDLES_VALIDITY_CONTEXT_ACCOUNT_SIZE } from '../constants';
import { ContextStateArgs, ProofDataInput } from './shared';

export interface VerifyGroupedCiphertext2HandlesValidityArgs {
 ...
 * 2. Calls the `VerifyGroupedCiphertext2HandlesValidity` instruction on the ZK ElGamal Proof program.
 */
export async function verifyGroupedCiphertext2HandlesValidity({
  ...
  if (contextState) {
    const space = BigInt(GROUPED_CIPHERTEXT_2_HANDLES_VALIDITY_CONTEXT_ACCOUNT_SIZE);
  ...
  const instructionInput: VerifyProofInput = {
    discriminator: ZkElGamalProofInstruction.VerifyGroupedCiphertext2HandlesValidity,
