...
export function getVerifyProofInstructionDataEncoder(): Encoder<VerifyProofInstructionDataArgs> {
  ...
  const write = (
    value: VerifyProofInstructionDataArgs,
    bytes: Uint8Array,
    offset: number
  ) => {
...
export type VerifyProofInstruction<
  ...
  TAccountContextStateAuthority extends string | AccountMeta<string> = string,
...
> {
  ...
  if (input.proofAccount) {
    ...
    accounts.push(getAccountMeta(originalAccounts.proofAccount)!);

    if (input.contextState) {
      accounts.push(getAccountMeta(originalAccounts.contextState)!);
      accounts.push(getAccountMeta(originalAccounts.contextStateAuthority)!);
    }
  } else {
  ...
  if (input.proofAccount) {
    ...
    if (!input.proofData) {
      throw new Error(
        'proofData is required when proofAccount is not provided'
      );
    }
