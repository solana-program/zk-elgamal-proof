...
impl ConfidentialKeys {
    ...
    pub fn signer_message(seed: Option<Uint8Array>) -> Result<Vec<u8>, JsValue> {
        if seed.is_some() {
            return Err(JsValue::from_str(
                "signerMessage takes no arguments, use signerMessageWithSeed",
            ));
        }
    ...
    pub fn prf_input(seed: Option<Uint8Array>) -> Result<Vec<u8>, JsValue> {
        if seed.is_some() {
            return Err(JsValue::from_str(
                "prfInput takes no arguments, use prfInputWithSeed",
            ));
        }
        Ok(STANDARD_DERIVATION_MESSAGE.to_vec())
    }
...
mod tests {
    ...
    #[wasm_bindgen_test]
    fn test_standard_messages_reject_a_seed() {
        for seed in [[7u8; 32].as_ref(), [].as_ref()] {
            let seed = Uint8Array::from(seed);
            assert!(ConfidentialKeys::signer_message(Some(seed.clone())).is_err());
            assert!(ConfidentialKeys::prf_input(Some(seed)).is_err());
        }

        assert!(ConfidentialKeys::signer_message(None).is_ok());
        assert!(ConfidentialKeys::prf_input(None).is_ok());
    }
