...
import { CIPHERTEXT_CIPHERTEXT_EQUALITY_CONTEXT_ACCOUNT_SIZE } from '../constants';
import { ContextStateArgs, ProofDataInput } from './shared';

export interface VerifyCiphertextCiphertextEqualityArgs {
 ...
 * 2. Calls the `VerifyCiphertextCiphertextEquality` instruction on the ZK ElGamal Proof program.
 */
export async function verifyCiphertextCiphertextEquality({
  ...
  if (contextState) {
    const space = BigInt(CIPHERTEXT_CIPHERTEXT_EQUALITY_CONTEXT_ACCOUNT_SIZE);
  ...
  const instructionInput: VerifyProofInput = {
    discriminator: ZkElGamalProofInstruction.VerifyCiphertextCiphertextEquality,
