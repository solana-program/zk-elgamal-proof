...
import { BATCHED_GROUPED_CIPHERTEXT_3_HANDLES_VALIDITY_CONTEXT_ACCOUNT_SIZE } from '../constants';
import { ContextStateArgs, ProofDataInput } from './shared';

export interface VerifyBatchedGroupedCiphertext3HandlesValidityArgs {
 ...
 * 2. Calls the `VerifyBatchedGroupedCiphertext3HandlesValidity` instruction on the ZK ElGamal Proof program.
 */
export async function verifyBatchedGroupedCiphertext3HandlesValidity({
  ...
  if (contextState) {
    const space = BigInt(BATCHED_GROUPED_CIPHERTEXT_3_HANDLES_VALIDITY_CONTEXT_ACCOUNT_SIZE);
