...
import { BATCHED_GROUPED_CIPHERTEXT_2_HANDLES_VALIDITY_CONTEXT_ACCOUNT_SIZE } from '../constants';
import { ContextStateArgs, ProofDataInput } from './shared';

export interface VerifyBatchedGroupedCiphertext2HandlesValidityArgs {
 ...
 * 2. Calls the `VerifyBatchedGroupedCiphertext2HandlesValidity` instruction on the ZK ElGamal Proof program.
 */
export async function verifyBatchedGroupedCiphertext2HandlesValidity({
  ...
  if (contextState) {
    const space = BigInt(BATCHED_GROUPED_CIPHERTEXT_2_HANDLES_VALIDITY_CONTEXT_ACCOUNT_SIZE);
